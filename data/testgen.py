...
for batch in range(batches):
	...
	for op in range(batchsize):
		k = token_hex(32)
		v = randint(0, 100)
		writefile.write("insert into testmultiple values("+str(k)+", "+str(v)+") on conflict(keycol) do update valcol = "+str(v)+";\n")
		if op%4 == 0:
			read2file.write("select * from testmultiple where keycol ="+str(k)+";\n")
		if op%10 == 0:
			k1 = token_hex(32)
			read1file.write("select * from testmultiple where keycol ="+str(k1)+";\n")
	writefile.close()
	read1file.close()
	read2file.close()
